...
void *teensyReaderFunction(void*) //void pointer
{
//	int fd_teensy = *(int*)p;
...
//	write(fd_teensy, &startLoop, 1);
	...
	while (keepGoing)
	{
		...
		memset(returnedValues, '\0', sizeof returnedValues); //set null character
		read(fd_teensy,returnedValues, sizeof returnedValues);
		
		if (returnedValues[0] != 0xff)// ensures that a slot is activated
		{
		...
		{
			pthread_mutex_lock( &slot_mutex );
			currentlyOccupiedSlot_GLOBAL = -1;//stored  in global variables and constants//allows us to modify this 
			pthread_mutex_unlock( &slot_mutex );
		}
